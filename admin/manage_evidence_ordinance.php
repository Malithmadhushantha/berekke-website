...
if (isset($_POST['delete_section']) && isset($_POST['section_id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("DELETE FROM user_bookmarks WHERE table_name = 'evidence_ordinance' AND section_id = ?");
        $stmt->execute([$section_id]);
        
        // Delete the section
        $stmt = $pdo->prepare("DELETE FROM evidence_ordinance WHERE id = ?");
        $stmt->execute([$section_id]);
...
if (isset($_POST['bulk_delete']) && isset($_POST['selected_ids'])) {
    ...
    if (!empty($selected_ids)) {
        try {
            ...
            // Delete related bookmarks first
            $stmt = $pdo->prepare("DELETE FROM user_bookmarks WHERE table_name = 'evidence_ordinance' AND section_id IN ($placeholders)");
            $stmt->execute($selected_ids);
            
            // Delete sections
            $stmt = $pdo->prepare("DELETE FROM evidence_ordinance WHERE id IN ($placeholders)");
            $stmt->execute($selected_ids);
...
// Get total count
$count_sql = "SELECT COUNT(*) FROM evidence_ordinance $where_clause";
$stmt = $pdo->prepare($count_sql);
$stmt->execute($search_params);
$total_sections = $stmt->fetchColumn();

// Get sections
$sections_sql = "SELECT * FROM evidence_ordinance $where_clause ORDER BY CAST(section_number AS UNSIGNED) ASC LIMIT $offset, $items_per_page";
...
$page_title = "Manage Evidence Ordinance - Admin";
...
<div class="container-fluid py-4">
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Manage Evidence Ordinance</li>
        </ol>
    </nav>
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h2>
                        ...
                        Manage Evidence Ordinance
                    </h2>
                    <p class="text-muted">Add, edit, and manage Evidence Ordinance sections</p>
                </div>
                <div>
                    <a href="add_evidence_section.php" class="btn btn-warning">
                        <i class="fas fa-plus me-1"></i>Add New Section
                    </a>
                    <a href="../evidence_ordinance.php" class="btn btn-outline-secondary">
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <form method="GET" action="" class="row g-3">
                        ...
                        <div class="col-md-2">
                            <a href="manage_evidence_ordinance.php" class="btn btn-outline-secondary w-100">
        ...
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    ...
                    <h4 class="mb-0">
                        <?php 
                        $bookmarked = $pdo->query("SELECT COUNT(*) FROM user_bookmarks WHERE table_name = 'evidence_ordinance'")->fetchColumn();
        ...
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    ...
                    <h4 class="mb-0">
                        <?php 
                        $notes = $pdo->query("SELECT COUNT(*) FROM user_bookmarks WHERE table_name = 'evidence_ordinance' AND notes IS NOT NULL AND notes != ''")->fetchColumn();
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    ...
                    <small class="text-muted">
                        <?php echo !empty($search_query) ? 'Search Results' : 'Evidence Rules'; ?>
                    </small>
                </div>
            </div>
        </div>
        ...
        <div class="col-12">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-light">
                    <h6 class="mb-0">
                        <i class="fas fa-bolt me-2"></i>Evidence Categories
                    </h6>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-2 mb-2">
                            <button class="btn btn-outline-warning btn-sm w-100" onclick="filterByCategory('direct')">
                                <i class="fas fa-eye me-1"></i>Direct Evidence
                            </button>
                        </div>
                        <div class="col-md-2 mb-2">
                            <button class="btn btn-outline-primary btn-sm w-100" onclick="filterByCategory('circumstantial')">
                                <i class="fas fa-puzzle-piece me-1"></i>Circumstantial
                            </button>
                        </div>
                        <div class="col-md-2 mb-2">
                            <button class="btn btn-outline-success btn-sm w-100" onclick="filterByCategory('document')">
                                <i class="fas fa-file-alt me-1"></i>Documentary
                            </button>
                        </div>
                        <div class="col-md-2 mb-2">
                            <button class="btn btn-outline-info btn-sm w-100" onclick="filterByCategory('hearsay')">
                                <i class="fas fa-ear-listen me-1"></i>Hearsay
                            </button>
                        </div>
                        <div class="col-md-2 mb-2">
                            <button class="btn btn-outline-danger btn-sm w-100" onclick="filterByCategory('admissions')">
                                <i class="fas fa-comment me-1"></i>Admissions
                            </button>
                        </div>
                        <div class="col-md-2 mb-2">
                            <button class="btn btn-outline-secondary btn-sm w-100" onclick="filterByCategory('burden')">
                                <i class="fas fa-balance-scale me-1"></i>Burden of Proof
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-warning text-dark"><?php echo htmlspecialchars($section['section_number']); ?></span>
                                ...
                                <td>
                                    <?php if (!empty($section['section_topic'])): ?>
                                        <span class="text-warning"><?php echo htmlspecialchars($section['section_topic']); ?></span>
                                    <?php else: ?>
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <a href="edit_evidence_section.php?id=<?php echo $section['id']; ?>" 
                                           class="btn btn-sm btn-outline-warning" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="../evidence_ordinance.php?search=<?php echo urlencode($section['section_number']); ?>" 
            ...
            <div class="text-center py-5">
                ...
                <p class="text-muted">
                    <?php if (!empty($search_query)): ?>
                        ...
                        <a href="manage_evidence_ordinance.php">view all sections</a>.
                    <?php else: ?>
                        Start by <a href="add_evidence_section.php">adding a new section</a>.
                    <?php endif; ?>
                </p>
            </div>
            <?php endif; ?>
