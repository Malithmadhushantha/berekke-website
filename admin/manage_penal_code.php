...
if (isset($_POST['delete_section']) && isset($_POST['section_id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("DELETE FROM user_bookmarks WHERE table_name = 'penal_code' AND section_id = ?");
        $stmt->execute([$section_id]);
        
        // Delete the section
        $stmt = $pdo->prepare("DELETE FROM penal_code WHERE id = ?");
        $stmt->execute([$section_id]);
...
if (isset($_POST['bulk_delete']) && isset($_POST['selected_ids'])) {
    ...
    if (!empty($selected_ids)) {
        try {
            ...
            // Delete related bookmarks first
            $stmt = $pdo->prepare("DELETE FROM user_bookmarks WHERE table_name = 'penal_code' AND section_id IN ($placeholders)");
            $stmt->execute($selected_ids);
            
            // Delete sections
            $stmt = $pdo->prepare("DELETE FROM penal_code WHERE id IN ($placeholders)");
            $stmt->execute($selected_ids);
...
// Get total count
$count_sql = "SELECT COUNT(*) FROM penal_code $where_clause";
$stmt = $pdo->prepare($count_sql);
$stmt->execute($search_params);
$total_sections = $stmt->fetchColumn();

// Get sections
$sections_sql = "SELECT * FROM penal_code $where_clause ORDER BY CAST(section_number AS UNSIGNED) ASC LIMIT $offset, $items_per_page";
...
$page_title = "Manage Penal Code - Admin";
...
<div class="container-fluid py-4">
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Manage Penal Code</li>
        </ol>
    </nav>
        ...
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted">Add, edit, and manage Penal Code sections</p>
                </div>
                <div>
                    <a href="add_penal_section.php" class="btn btn-primary">
                        <i class="fas fa-plus me-1"></i>Add New Section
                    </a>
                    <a href="../penal_code.php" class="btn btn-outline-secondary">
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <form method="GET" action="" class="row g-3">
                        ...
                        <div class="col-md-2">
                            <a href="manage_penal_code.php" class="btn btn-outline-secondary w-100">
        ...
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    ...
                    <h4 class="mb-0">
                        <?php 
                        $bookmarked = $pdo->query("SELECT COUNT(*) FROM user_bookmarks WHERE table_name = 'penal_code'")->fetchColumn();
        ...
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    ...
                    <h4 class="mb-0">
                        <?php 
                        $notes = $pdo->query("SELECT COUNT(*) FROM user_bookmarks WHERE table_name = 'penal_code' AND notes IS NOT NULL AND notes != ''")->fetchColumn();
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    ...
                    <small class="text-muted">
                        <?php echo !empty($search_query) ? 'Search Results' : 'Showing'; ?>
                    </small>
                </div>
            </div>
        </div>
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-primary"><?php echo htmlspecialchars($section['section_number']); ?></span>
                                ...
                                <td>
                                    <?php if (!empty($section['section_topic'])): ?>
                                        <span class="text-info"><?php echo htmlspecialchars($section['section_topic']); ?></span>
                                    <?php else: ?>
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <a href="edit_penal_section.php?id=<?php echo $section['id']; ?>" 
                                           class="btn btn-sm btn-outline-primary" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="../penal_code.php?search=<?php echo urlencode($section['section_number']); ?>" 
            ...
            <div class="text-center py-5">
                ...
                <p class="text-muted">
                    <?php if (!empty($search_query)): ?>
                        ...
                        <a href="manage_penal_code.php">view all sections</a>.
                    <?php else: ?>
                        Start by <a href="add_penal_section.php">adding a new section</a>.
                    <?php endif; ?>
                </p>
            </div>
            <?php endif; ?>
